...
客户端登录
    ...
    delay  0表示不延迟，立即进行登录 1表示在after_load_plugin事件触发后执行，即全部插件加载完毕后再执行登录
...
注意： 由于需要发送邮件附件，依赖模块 L<Mojo::SMTP::Client> L<MIME::Lite>，需要单独安装
    ...
    #插件需要使用到 input_img_verifycode 事件，需要设置为延迟登录
...
提供HTTP API接口，方便获取客户端帐号、好友、群、讨论组信息，以及通过接口发送好友消息、群消息、群临时消息和讨论组临时消息
    ...
    $client->load("Openqq",data=>{                                                 
        ...
        auth   => sub {my($param,$controller) = @_},    #认证回调函数，用于进行请求鉴权
    });
