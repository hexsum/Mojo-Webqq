...
登录需要扫描二维码，参数分别为二维码文件的本地路径，二维码原始下载地址
...
使用方法和KnowledgeBase2基本完全一致，存在的差别：
...
调用系统的图片查看程序来展示二维码图片，方便扫描，作者 autodataming L<https://github.com/autodataming>
    ...
    $client->load("ShowQRcode");#当前仅支持win,并且需要在调用$client->login()方法之前加载
    $client->login();
    $client->load("ShowMsg");
    ...;
