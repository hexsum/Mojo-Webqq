...
A Webqq Client Framework base on Mojolicious
...
此项目是Webqq-Client模块的重构，基于L<Mojolicious>框架，具体更多良好特性，比如：
...
所以的类都继承Mojo::Webqq::Base具有dump的方法，这些数据类型均由系统自动创建和维护，一般情况下，你只需要对对象进行只读操作
...
=item Mojo::Webqq::Recent::Discuss
...
消息类似主要包含好友消息、群消息、讨论组消息、群临时消息、讨论组临时消息

所有的消息类都继承Mojo::Webqq::Message::Base，具备dump的方法
...
基于Mojo::EventEmitter的事件注册方法，可支持同时设置多个事件的回调
...
客户端进入事件循环，正式开始运行，一般放在代码的最后，不可获取
...
=head2 send_message($friend,$content)
...
=head2 send_group_message($group,$content)
...
=head2 send_discuss_message($discuss,$content)
...
=head2 send_sess_message($group_member|$discuss_member,$content)
...
=head2 reply_message($msg,$content)
...
但如果是非管理员权限,则完全不会收到任何通知提醒，以新增群成员为例
...
加载一个或者多个插件，多个插件使用数组引用，支持的插件参数包括:
    ...
    priority        #设置插件优先级，默认是0，较高的优先级能够使得插件优先执行
    auto_call       #设置是否加载完成后自动执行，默认为1
    data            #设置加载插件时可以携带的数据，将会在call的时候传递给插件本身
...
比如一个插件对消息处理完并不希望其他插件再继续处理该消息（默认情况下，receive_message事件会广播个所有订阅该事件的回调）
