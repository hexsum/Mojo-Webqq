...
=head2 new()
    ...
    state           #设置登录状态，默认是online，支持online|away|busy|silent|hidden|offline|callme
...
=head2 Mojo::Webqq::Plugin::PostQRcodeToTelegram

登录过程如果需要手机扫描二维码，会将二维码通过Telegram Bot发至指定账户或频道或群组，再通过手机QQ扫描二维码

    $client->load("PostQRcodeToTelegram",data=>{
        api_key    =>  'xxxx', #Telegram Bot的API Key
        chat_id    =>  'xxxx', #接收信息的chat_id
    });
