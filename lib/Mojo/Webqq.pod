...
需要依赖模块 L<Mojo::IRC::Server::Chinese>
    ...
    $client->load("IRCShell",data=>{
        ...
        load_friend => 1, #默认是1 是否初始为每个好友生成irc虚拟帐号并加入频道 #我的好友
        master_irc_nick => "你的QQ昵称", #用于识别哪个irc用户是你自己
    });
    #支持的参数包括：
    ...
    master_irc_nick     #和qq匹配的irc帐号昵称，默认按照和qq号相同的昵称或者客户端ip是本机地址作为识别规则
