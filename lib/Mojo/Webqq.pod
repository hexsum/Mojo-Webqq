...
=head1 SYNOPSIS
    ...
    #手动设置输出编码参加文档中关于 log_encoding 的说明
...
注意基于web的方式没办法批量获取到好友、群成员或讨论组成员的qq号码，只能够使用api实时每次查询单个用户的qq号码

这种情况下，如果程序不注意，会产生较大的http请求，并且获取qq号码的效率也会很低

虽然程序当前对每个用户查询的qq结果会进行缓存，但还是建议避免在程序使用到qq号，尤其是在搜索过程中，比如:

    $client->search_friend(qq=>xxxxx); 会导致遍历好友，使用腾讯的api查询每个好友id对应的qq号
...
=head2 Mojo::Webqq::Group 群组
    ...
    $group->me->displayname #获取自己在群中的显示名称
...
=head2 Mojo::Webqq::Message::Recv::Message 接收到的好友消息
    ...
    方法：
    dump            #打印消息结构，主要用于调试
    to_json         #将消息转换为json兼容的hash，方便和外部系统交互
    ...
    my $json_hash = $msg->to_json();    #获取到经过utf8 decode的hash引用
    my $json_text = encode_json($json); #将hash转换为json字符串
...
=head2 new()
    ...
    login_type      #"qrlogin"表示使用二维码登录，"login"表示帐号密码登录，默认为"login"
    ...
    friend_pic_dir  #设置好友图片保留目录，默认和pic_dir一致，只有注册了receive_friend_pic事件 此路径才有意义
    group_pic_dir   #设置群图片保留目录，默认和pic_dir一致，只有注册了receive_group_pic事件 此路径才有意义
    pic_dir         #图片接收默认地址，默认为空，即不下载保存图片
...
=head2 receive_group_pic

接收到群图片，图片默认下载到临时目录下

    $client->on(receive_group_pic=>sub{ 
        my($client,$fh,$filepath,$group,$sender)=@_;
        #$fh       图片文件的只读句柄
        #$filepath 图片文件的绝对路径
        #$group    发送图片的群对象
        #$sender   发送图片的群成员对象
