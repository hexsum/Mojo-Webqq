...
注意：推荐安装L<Crypt::RSA> 或者 L<Crypt::OpenSSL::RSA>和L<Crypt::OpenSSL::Bignum>模块，能够极大的提升登录加密算法的计算速度
...
=head2 mail 客户端阻塞或非阻塞发送邮件

该方法依赖Mojo::SMTP::Client模块，具体使用方法请参照该模块
...
实现QQ群和IRC的联通，彼此的消息自动同步，欢迎加入irc.freenode.net的#ChinaPerl，该频道已经和[PERL学习交流]群联通
    
    $group1 = $client->search_group(gname=>"PERL学习交流");
    $group2 = $client->search_group(gname=>"xxx");
    $client->load("MsgSync",data=>[
        {nick=>"xxx_",user=>"xxx",pass=>"xxx",channel=>"#ChinaPerl"},
        $group1,
        $group2]
    );
...
实现通过QQ消息查询perldoc文档，支持perldoc -f|-v xxx
    ...
    $client->load("Perldoc",priority=>10);
...
通过QQ消息执行Perl代码，仅支持在linux系统上使用
    ...
    $client->load("Perlcode",priority=>11);
    #由于该插件处理完的消息不应该再由其他插件处理，因此插件优先级应该设置成比其他插件优先级更高，插件默认优先级是0
...
为避免对群内成员产生影响，群内需要使用 @帐号昵称 来触发
    ...
    $client->load("SmartReply",priority=>7);
...
调用示例
    ...
    http://127.0.0.1:5000/send_message?id=xxx&content=hello (中文需要utf8编码，且需要做urlencode)
