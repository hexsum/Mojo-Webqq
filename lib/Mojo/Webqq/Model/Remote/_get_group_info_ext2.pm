use strict;
sub Mojo::Webqq::Model::_get_group_info_ext2 {
    my $self = shift;
    my $uid = shift;
    my $callback = shift;
    ...
    if($is_blocking){
        return $handle->( $self->http_post($api,{Origin=>"http://qun.qq.com",Referer=>"http://qun.qq.com/member.html",json=>1},form=>{gc=>$uid,st=>0,end=>2000,sort=>0,bkn=>$self->get_csrf_token},) );
    }
    else{
        $self->http_post($api,{Origin=>"http://qun.qq.com",Referer=>"http://qun.qq.com/member.html",json=>1},form=>{gc=>$uid,st=>0,end=>2000,sort=>0,bkn=>$self->get_csrf_token},sub{
            my $json = shift;
